...
test('REEXP risk copy', async ({ page }) => {
  ...
  for (const row of rowsToRun) {
    ...
    const idLocator = page.locator("li.tab.active a.tab-link");
    await expect(idLocator).toBeVisible({ timeout: 10000 });

    const tabText = (await idLocator.innerText()).trim();
    console.log("Tab text:", tabText);

    // Extract pattern: 4 letters + 3 digits
    const match = tabText.match(/^[A-Za-z]{4}\d{3}$/);

    if (!match) {
      throw new Error("Risk ID does not match pattern (AAAA999)");
    }

    const riskId = match[0];
    console.log("Extracted Risk ID:", riskId);
    ...
    // Use ID in UI search
    await page.goto("https://www.landydev.com/#/pages/policyQueued");
    await page.fill("#globalSearch", riskId);
    await page.click("(//button[@id='search'])[1]");

    // // verify
    // await expect(page.locator(".result-id")).toHaveText(riskId);

    await page.locator("(//input[@type='checkbox'])[14]").click();
