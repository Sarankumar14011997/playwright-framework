...
test('ACEXP-Direct Excel', async ({ page }) => {
    ...
    // Load ACEXP
    // @ts-ignore
    const rows = await readExcel(file, "ACEXP");
    console.log("Rows from ACEXP:", rows.length);
    ...
    for (const row of rowsToRun) {
        ...
        // await page.locator('//input[@id="globalSearch"]').fill('ACEX002');
        ...
        await page.waitForTimeout(1000);
        await page.locator('//input[@id="totNoOfProf"]').fill(row.totNoOfProf || '');


        const CurrentFiscalYear = page.locator('(//input[@placeholder=\'$0.00\'])[1]');
        await CurrentFiscalYear.click();
        await CurrentFiscalYear.fill(""); // clear any old value
        await CurrentFiscalYear.type(row.CurrentFiscalYear?.toString() || "0", { delay: 100 });
        await page.waitForLoadState("networkidle", { timeout: 5000 });

        const LastFiscalYear = page.locator('(//input[@placeholder=\'$0.00\'])[2]');
        await LastFiscalYear.click();
        await LastFiscalYear.fill(""); // clear any old value
        await LastFiscalYear.type(row.LastFiscalYear?.toString() || "0", { delay: 100 });
        await page.waitForLoadState("networkidle", { timeout: 5000 });

        const PreviousFiscalYear = page.locator('(//input[@placeholder=\'$0.00\'])[3]');
        await PreviousFiscalYear.click();
        await PreviousFiscalYear.fill(""); // clear any old value
        await PreviousFiscalYear.type(row.PreviousFiscalYear?.toString() || "0", { delay: 100 });
        await page.waitForLoadState("networkidle", { timeout: 5000 });


        // APPLICATION DETAILS SAVE & CLOSE
        ...
        await page.locator("//label[@for='defaultInline_2_15_AccExpPremiumTable']").click();
        ...
        //(FIRST RISK ID PICK)
        await page.locator("tbody tr[class='ng2-smart-row selected ng-star-inserted'] td:nth-child(3) ng2-smart-table-cell:nth-child(1) table-cell-view-mode:nth-child(1) div:nth-child(1) div:nth-child(1)").click();

        // await page.locator("//div[contains(text(),'ACEX005')]").click();

        await page.waitForLoadState('networkidle', { timeout: 5000 });
        await page.locator("//nb-tab[2]//nb-accordion/nb-accordion-item[1]/nb-accordion-item-header").click();
        await page.waitForLoadState('networkidle', { timeout: 5000 });
        await page.locator("(//button[@id='renewalResponse1'])[1]").click();
        await page.waitForLoadState('networkidle', { timeout: 10000 });
        await page.locator("//button[@id='sendMail']").click();
        await page.waitForTimeout(4000);
        await page.waitForLoadState('networkidle', { timeout: 10000 });
        await page.locator("//body/ngx-app[1]/ngx-pages[1]/ngx-one-column-layout[1]/nb-layout[1]/div[1]/div[1]/div[1]/div[1]/div[1]/nb-layout-column[1]/ngx-underwriter[1]/div[1]/nb-card[1]/nb-tabset[1]/ul[1]/li[1]/a[1]").click();
        await page.waitForLoadState('networkidle', { timeout: 5000 });

        //ACTIONS BUTTON
        await page.locator("//button[@id='dropdownBasic1']").click();
        await page.locator("//button[@id='renewal']").click();

        // RENEWAL DATE
        const RenewalApplicationDate = new Date().toISOString().split('T')[0];
        await page.locator('#appReceived2').fill(RenewalApplicationDate);
        await page.locator('#appReceived2').press('Tab');

        // UPLOAD DOCUMENT
        await page.setInputFiles('#file', 'D:/Landydev_playwright/tests/uploads/sample.pdf');
        await page.waitForLoadState('networkidle', { timeout: 5000 });
        // await expect(page.locator('text=sample.pdf')).toBeVisible();
        await page.locator("//button[@id='save1']").click();
        await page.waitForLoadState('networkidle', { timeout: 15000 });
        await page.waitForTimeout(5000);
        


        // ENDORSEMENTS FOR RENEWAL RISK
        console.log("Step 8: Endorsements Test Started");
        await page.locator("//nb-accordion-item-header[.//span[normalize-space(text())='Endorsements']]").click();
        await page.waitForLoadState('networkidle', { timeout: 4000 });

        // ENDORSEMENTS FOR RENEWAL RISK SAVE & CLOSE
        const EndorsementSaveButton1 = page.locator("//button[@id='saveClose']");
        await expect(EndorsementSaveButton1).toBeEnabled({ timeout: 20000 });
        await EndorsementSaveButton1.click();
        await page.waitForLoadState('networkidle', { timeout: 4000 });


        // // Open Endorsements section
        // const EndorsementSaveButton1 = page.locator("//nb-accordion-item-header[.//span[contains(.,'Endorsements')]]/parent::nb-accordion-item//button[@id='saveClose']");
        // await EndorsementSaveButton1.waitFor({ state: "visible", timeout: 20000 });
        // await expect(EndorsementSaveButton1).toBeEnabled();
        // await EndorsementSaveButton1.click();

        // ENDORSEMENTS FOR RENEWAL RISK NEXT 
        await page.locator("//button[@id='next']").click();
        await page.waitForFunction(() => document.readyState === 'complete', null, { timeout: 5000 });
        await waitForFullPageLoad(page);


        //PURCHASED OPTIONS PAGE FOR RENEWAL RISK
        console.log("Step 9: Purchased Options Test Started");
        ...
        await page.waitForSelector("tr[class='bold ng-star-inserted'] td:nth-child(9)", { timeout: 15000 });// 1) Wait for the table to appear
        const rawBalance1 = await page.locator("tr[class='bold ng-star-inserted'] td:nth-child(9)").first().innerText(); // 2) Capture the balance from the row

        // 3) Normalize the captured text to a numeric string, "$597.00" -> "597.00"
        const normalized1 = rawBalance1.replace(/\s/g, '')           // remove whitespace
            .replace(/\$/g, '')          // remove dollar sign
            .replace(/,/g, '')           // remove commas
            .replace(/[^\d.()-]/g, ''); // keep digits, dot, minus/parens

        console.log('Captured balance:', rawBalance1, '=>', normalized1);

        // 4) Wait for Amount Received input - try a few common locators and pick the first that exists
        const amountLocators1 = [
            "//input[@placeholder='Amount Received']",
            "//input[@formcontrolname='amountReceived']",
            "//input[contains(@id,'amount') and contains(@id,'received')]",
            "//label[normalize-space()='Amount Received']/following::input[1]",
        ...
        let amountInput1 = null;
        for (const sel of amountLocators1) {
            const loc = page.locator(sel);
            if (await loc.count() > 0) {
                amountInput1 = loc.first();
                break;
            }
        }
        if (!amountInput1) {
        ...
        await amountInput1.fill('');              // clear any value
        await amountInput1.type(normalized, { delay: 50 }); // simulate user typing
        await page.keyboard.press('Tab');        // trigger blur / validation
        ...
        // //FIRTST RISK ID PICK
        // await page.locator("//div[contains(text(),'ACEX003')]").click();
        ...
        console.log("Step 8: cancellation Endorsements Test Started");
        await page.locator("//nb-accordion-item-header[.//span[normalize-space(text())='Endorsements']]").click();
        await page.waitForTimeout(2000);
        await page.locator("//ng-multiselect-dropdown[@name='endosements']//div[@class='multiselect-dropdown']//div//span[@class='dropdown-btn']").click();

        await page.locator("//div[normalize-space()='D44406 (05/13) Cancellation']").click();
        await page.locator("//ng-multiselect-dropdown[@name='endosements']//div[@class='multiselect-dropdown']//div//span[@class='dropdown-btn']").click();

        await page.locator("//i[@title='Add Relevant Data']").click();
        await page.locator("label[for='customCheck1']").click();
        await page.locator("//button[@id='save']").click();
        await page.waitForTimeout(2000);

        // CANCELLATION ENDORSEMENTS SAVE & CLOSE
        const EndorsementSaveButton3 = page.locator("//button[@id='saveClose']");
        await expect(EndorsementSaveButton3).toBeEnabled();
        await EndorsementSaveButton3.click();
        await page.waitForLoadState('networkidle', { timeout: 4000 });


        // MIDTERM EFFECTIVE DATE
        await page.locator("//button[@id='midTermEffectiveDateDialog']").click();



        await page.fill('#midTermEffDate', '2026-11-27');
        await page.locator('#midTermEffDate').press('Tab');


        // async function selectDateFromCalendar(page, selector, day) {
        ...
        //     // Click the date
        //     await page.locator(`//nb-calendar-day-cell//*[text()='${day}']`).click();
        // }

        // // Use it:
        // await selectDateFromCalendar(page, "#midTermEffDate", "25");




        await page.locator("label[for='checkFlag1']").click();// CHECKBOX

        // Today in YYYY-MM-DD format
        const pastMidTermEffectiveDate = new Date().toISOString().split('T')[0];
        await page.locator('#pastMidTermEffectiveDate').fill(pastMidTermEffectiveDate);
        await page.locator('#pastMidTermEffectiveDate').press('Tab');

        await page.locator("//button[@id='onClickMidTermEffectiveDateModal']").click();
        await page.waitForLoadState('networkidle', { timeout: 4000 });
        ...
        await page.locator('#cancelReason0').first().selectOption({ label: 'Non-Payment of Premium' });
        await page.locator('#cancelReason0').last().selectOption({ label: 'Moving to Another Company' });



        await page.locator("//label[@for='masterSelected']").click();

        await page.locator("label[for='masterSelectedFop']").click();
        await page.waitForLoadState('networkidle', { timeout: 4000 });
